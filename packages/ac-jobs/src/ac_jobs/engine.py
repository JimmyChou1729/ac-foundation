...
class _SnapshotStore:
    ...
    def _encode(self, value: RunSnapshot) -> dict[str, JsonValue]:
        return {
            "schema_version": "ac.jobs.run_snapshot.v3",
            "run_id": value.run_id,
    ...
    def _decode(self, document: Mapping[str, JsonValue]) -> RunSnapshot:
        ...
        if schema_version == "ac.jobs.run_snapshot.v3":
            required.add("recovery_epoch")
        elif schema_version != "ac.jobs.run_snapshot.v2":
...
class RunRepository:
    ...
    def _spec_document(self, spec: RunSpec) -> dict[str, JsonValue]:
        ...
        projection = {
            "schema_version": "ac.jobs.run_semantic_key.v1",
            "handler": spec.handler,
            "semantic_input": dict(spec.semantic_input),
        }
        return {
            "schema_version": "ac.jobs.run_spec.v1",
            "run_id": spec.run_id,
            "handler": spec.handler,
            "semantic_input": dict(spec.semantic_input),
            "semantic_key_schema": "ac.jobs.run_semantic_key.v1",
    ...
    def read_spec(self, run_id: str) -> RunSpec:
        ...
        if document["schema_version"] != "ac.jobs.run_spec.v1":
...
class RunEngine:
    ...
    def _persist_resume_input(
        ...
        document: dict[str, JsonValue] = {
            "schema_version": "ac.jobs.resume_input_record.v1",
        ...
        *,
        ...
        try:
            ...
            atomic_write_json(
                ...
                {
                    "schema_version": "ac.jobs.lease.v1",
                    "attempt": snapshot.attempt,
                    "acquired_at": utc_now(),
                    "pid": os.getpid(),
