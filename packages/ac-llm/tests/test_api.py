...
import ac_llm.api as api_module
from ac_jobs import (
    AtomicStateStore,
    ArtifactDigest,
    ArtifactSourceRef,
...
from ac_jobs.lease import FileLease
from ac_llm import (
...
from ac_llm.identity import canonical_json_bytes, semantic_key
from ac_llm.output import CandidateMaterial
from ac_llm.executor import HANDLER_NAME, LLMTaskExecutor
...
def test_content_rich_invalid_output_uses_formatter_without_worker_replacement(
    ...
    for path in tmp_path.rglob("*"):
        ...
        if (
            ...
            and document.get("schema_version") == "ac.llm.output_formatting.v1"
        ):
...
def test_new_standalone_run_writes_only_fixed_closed_invocation_v2(
    ...
    assert document["schema_version"] == "ac.jobs.state.v1"
    assert (
        document["contract_schema_version"]
        == "ac.llm.standalone_invocation.v2"
    )
