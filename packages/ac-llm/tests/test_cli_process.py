...
def test_cli_emits_one_shared_envelope_and_query_status(
    ...
    assert code == 0
    assert generated["schema_version"] == "ac.command_result.v2"
...
def test_cli_stderr_progress_and_stdout_final_result(
    ...
    assert json.loads(final_lines[0])["schema_version"] == "ac.command_result.v2"
    progress = [json.loads(line) for line in captured.err.splitlines()]
    assert progress
    assert all(item["schema_version"] == "ac.progress_event.v1" for item in progress)
