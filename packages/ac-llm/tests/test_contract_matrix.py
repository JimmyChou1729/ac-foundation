...
from ac_llm.errors import CandidateConflictError, OutputInvalidError
...
from ac_llm.output import CandidateMaterial, candidate_digest, select_output
from ac_llm.providers._cli import EventAccumulator
from ac_llm.providers.claude import _parse_event as parse_claude_event
from ac_llm.providers.codex import _parse_event as parse_codex_event
...
def test_config_resolution_matrix_is_explicit_and_host_deterministic() -> None:
    explicit = detect_host(
        env={
            "AC_AGENT_HOST": "kimi",
            "AC_LLM_PROVIDER": "ignored",
            "CODEX_THREAD_ID": "ignored",
        },
        process_chain=("claude --print",),
    )
    assert (explicit.host, explicit.evidence) == ("kimi", "AC_AGENT_HOST")
    ...
    env_cases = (
        ...
        ({"DSH_AC_LLM_SOCKET": "/tmp/bridge.sock"}, "dsh", "DSH_AC_LLM_SOCKET"),
    )
    ...
    exact = resolve_model_selection(
        ...
        env={"AC_LLM_MODEL": "ignored", "AC_CODEX_MODEL": "ignored"},
    )
...
def test_public_import_exposes_only_the_supported_facades_and_lazy_adapters() -> None:
    ...
    env["PYTHONPATH"] = os.pathsep.join(
        (
            str(root / "packages/ac-jobs/src"),
            str(root / "packages/ac-llm/src"),
        )
    )
    source = """
import json
...
import ac_llm
print(json.dumps({
    "exports": sorted(ac_llm.__all__),
    "modules": sorted(name for name in sys.modules if name.startswith("ac_")),
    "version": ac_llm.__version__,
}))
"""
    ...
    assert not any(
        name.startswith("ac_proposer_reviewer") for name in observed["modules"]
    )
    assert not {
        "ac_llm.providers.codex",
        "ac_llm.providers.claude",
        "ac_llm.providers.kimi",
...
def test_explicit_run_root_is_the_only_llm_storage_root(
    ...
    monkeypatch.setenv("AC_HOME", str(shadow))
    monkeypatch.setenv("AC_LLM_CACHE", str(shadow / "cache"))
    monkeypatch.setenv("AC_LLM_TMP_DIR", str(shadow / "tmp"))
