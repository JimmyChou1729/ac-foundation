...
def _parser() -> _Parser:
    ...
    exported.add_argument("--pdf-source-manifest")
    _cache_root(exported)

    mineru = commands.add_parser("import-mineru-bundle")
    mineru.add_argument("pdf")
    mineru.add_argument("--content-list", required=True)
    mineru.add_argument("--middle-json", required=True)
    mineru.add_argument("--output-dir", required=True)

    configured = commands.add_parser("configure-mineru")
    configured.add_argument("--config-path", required=True)
    configured.add_argument("--executable")
    configured.add_argument("--api-url")
    configured.add_argument("--token-env")
    configured.add_argument("--language", choices=("en", "ch"), default="en")
    config_doctor = commands.add_parser("doctor-configured-mineru")
    config_doctor.add_argument("--config-path", required=True)
    config_parse = commands.add_parser("parse-pdf-configured-mineru")
    config_parse.add_argument("pdf")
    config_parse.add_argument("--config-path", required=True)
    config_parse.add_argument("--job-dir", required=True)
    config_parse.add_argument("--timeout-seconds", type=float, default=900)

    doctor = commands.add_parser("doctor-mineru")
    run_mineru = commands.add_parser("parse-pdf-mineru")
    run_mineru.add_argument("pdf")
    run_mineru.add_argument("--job-dir", required=True)
    run_mineru.add_argument("--language", choices=("en", "ch"), default="en")
    run_mineru.add_argument("--timeout-seconds", type=float, default=900)
    for command in (doctor, run_mineru):
